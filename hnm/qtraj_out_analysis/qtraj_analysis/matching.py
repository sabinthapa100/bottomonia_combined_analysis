...
def build_observables(
    ...
    for r in records:
        ...
        if r.L in grouped[key]:
            duplicate_counts[r.L] = duplicate_counts.get(r.L, 0) + 1
        else:
            grouped[key][r.L] = r

    if duplicate_counts:
        by_l = ", ".join(
            f"L={lval}: {count}" for lval, count in sorted(duplicate_counts.items())
        )
        logger.warning(
            "Found duplicate records (keeping first per meta-key/L). Counts -> %s",
            by_l,
        )
