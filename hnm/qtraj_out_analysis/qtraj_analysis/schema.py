...
class Record:
    ...
    One 2-row record from the datafile:
      ...
      vec:  mapped vector from second row (length 8):
            [v1..v6, L, qweight_like]

    In the Mathematica-consistent production path, ``qweight_like`` is
    the averaged-row multiplicity ``Ntraj`` from ``datafile-avg.gz``.
    Raw 8-column qtraj rows should be averaged first via
    ``load_qtraj_table()`` / ``processEvents.py`` before constructing
    ``Record`` objects.
    """
    meta: np.ndarray
    vec: np.ndarray
