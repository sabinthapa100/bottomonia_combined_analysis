    ...
    if is_torch or isinstance(mu, np.ndarray):
        ...
        if method == "asym":
             if is_torch:
                 ...
                 res = np.array([_asym_series(float(m), cfg) for m in mu.ravel()])
        ...
        if is_torch:
            ...
            res = np.array([_ode_solve(float(m), cfg) for m in mu.ravel()])
            return res.reshape(mu.shape)

    return _alpha_s_cached(mu, Nf, loops, muRef, alphaRef, LambdaQCD, method, freeze, alpha_max)

@lru_cache(maxsize=8192)
def _alpha_s_cached(mu: float, Nf, loops, muRef, alphaRef, LambdaQCD, method, freeze, alpha_max):
    cfg = AlphaSConfig(Nf=Nf, loops=loops, muRef=muRef, alphaRef=alphaRef,
                       LambdaQCD=LambdaQCD, method=method,
                       freeze=freeze, alpha_max=alpha_max)
    if method == "asym":
