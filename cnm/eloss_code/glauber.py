...
class HarmonicOscillator:
    """
    Harmonic Oscillator (HO) nuclear density for light nuclei (e.g. Oxygen A=16).
    Default parameters from De Vries et al. (1987), as implemented in Nucleus.cxx.
    ρ(r) = ρ₀ (1 + α · r²/a²) exp(-r²/a²)
    Normalized so that ∫ d³r ρ(r) = A.
    """
    A: int = 16
    aHO: float = 1.833     # fm (HO length scale)
    alphaHO: float = 1.544  # dimensionless shape parameter
    rho0: float = DEFAULT_RHO0
    rmax_fm: float = 30.0
    dr_fm: float = 0.02
    zmax_fm: float = 30.0
    ...
    def rho(self, r: np.ndarray) -> np.ndarray:
        ...
        a, alpha = self.aHO, self.alphaHO
        rho_raw = self.rho0 * (1.0 + alpha * (r**2 / a**2)) * np.exp(-r**2 / a**2)
        # Analytical normalization: ∫d³r ρ_raw = π^{3/2} ρ₀ a³ (1 + 3α/2)
        norm = (math.pi ** 1.5) * self.rho0 * (a ** 3) * (1.0 + 1.5 * alpha)
        return (float(self.A) / norm) * rho_raw
    ...
    def tabulate_T_of_r(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for i, ri in enumerate(r):
            rr = np.sqrt(ri * ri + z * z)
            T[i] = self.zmax_fm * np.sum(w * self.rho(rr))
        return r, T


@dataclass(frozen=True)
class ThreeParamFermi:
    """
    Three-parameter Fermi (3pF) nuclear density for light nuclei.
    ρ(r) = ρ₀ (1 + w·r²/c²) / (1 + exp((r-c)/z))
    Default parameters for Oxygen from De Vries et al. (1987).
    """
    A: int = 16
    c_fm: float = 2.608    # half-density radius [fm]
    z_fm: float = 0.513    # surface diffuseness [fm]
    w: float = -0.051      # deformation parameter
    rho0: float = DEFAULT_RHO0
    rmax_fm: float = 30.0
    dr_fm: float = 0.02
    zmax_fm: float = 30.0
    ...
    def rho(self, r: np.ndarray) -> np.ndarray:
        rho_raw = self.rho0 * (1.0 + self.w * (r**2 / self.c_fm**2)) / \
                  (1.0 + np.exp((r - self.c_fm) / self.z_fm))
        # Normalize numerically: integrate rho_raw over all space
        r_int = np.linspace(0, self.rmax_fm, 2000)
        rho_int = self.rho0 * (1.0 + self.w * (r_int**2 / self.c_fm**2)) / \
                  (1.0 + np.exp((r_int - self.c_fm) / self.z_fm))
        norm = 4.0 * math.pi * np.trapz(r_int**2 * rho_int, r_int)
        return (float(self.A) / max(norm, 1e-12)) * rho_raw
...
class OpticalGlauber:
    def __init__(
        ...
        *,
        ...
        # target thickness — auto-select density profile
        if density_name == "HarmonicOscillator" or (density_name is None and spec.A == 16):
            self.density_profile = HarmonicOscillator(A=spec.A)
            if self.verbose:
                print(f"[OpticalGlauber] Using Harmonic Oscillator density for A={spec.A}")
        elif density_name == "ThreeParamFermi":
            self.density_profile = ThreeParamFermi(A=spec.A)
            if self.verbose:
                print(f"[OpticalGlauber] Using Three-Parameter Fermi density for A={spec.A}")
        elif density_name == "WoodsSaxon":
            self.density_profile = WoodsSaxon(A=spec.A, d_fm=self.d_fm)
            if self.verbose:
                print(f"[OpticalGlauber] Using Woods-Saxon density for A={spec.A}")
        else:
            self.density_profile = WoodsSaxon(A=spec.A, d_fm=self.d_fm)
            if self.verbose and spec.A != 16 and density_name is not None:
                print(f"[OpticalGlauber] Fallback to Woods-Saxon density for A={spec.A}")
        self.ws = self.density_profile  # backward compat alias
        self.r_grid, self.TA_r_lut = self.density_profile.tabulate_T_of_r()
    ...
    def _leff_bin_dA_binomial(
        ...
        for N in range(N1, N2+1):
            ...
            # Arleo Eq B.13 assumes T_A integrates to 1. Our TA0 integrates to A, so we use (TA0/A).
            
            term_num_b += (N*(N-1)) * ((TA0/float(A))**2 * I_num_r)
            term_den_b += N * ((TA0/float(A)) * I_den_r)
    ...
    # =============================================================================
    # AA L_eff (Binomial 1-sided pA method, following Arleo-Peigne)
    # =============================================================================

    def leff_bin_AA(self, c0: float, c1: float, *,
                    rho0_fm3: float = DEFAULT_RHO0) -> float:
        """
        Effective path length L_eff for AA collisions in centrality bin [c0, c1].
        For symmetric AA collisions, L_A = L_B = L_eff.
        Following Arleo & Peigne, the AA cross section factorizes into two independent
        pA-like convolutions. Therefore, the L_eff for nucleus A is exactly the pA
        L_eff calculated as if one nucleus is the projectile and the other is the target.
        """
        return self.leff_bin_pA(c0, c1, rho0_fm3=rho0_fm3, method="binomial")

    def leff_minbias_AA(self, *, rho0_fm3: float = DEFAULT_RHO0) -> float:
        """Minimum-bias L_eff for AA collisions."""
        return self.leff_bin_pA(0.0, 1.0, rho0_fm3=rho0_fm3, method="binomial")
    ...
    def leff_bins_AA(
        ...
        *,
        ...
        """Return {"0-10%": L_eff, ...} for AA bins (percent)."""
        bins, labels = self._parse_cent_bins(cent)
        out: Dict[str, float] = {}
        for (a, b), lab in zip(bins, labels):
            out[lab] = float(self.leff_bin_AA(a / 100.0, b / 100.0, rho0_fm3=rho0_fm3))
        return out

    def print_table_AA(self, edges_percent: Sequence[float]):
        """Pretty-print AA centrality table."""
        import textwrap
        leff = self.leff_bins_AA(edges_percent)
        hdr = f"{'Cent':>12s}  {'L_eff[fm]':>10s}"
        rows = [hdr, "-" * len(hdr)]
        for lab, L in leff.items():
            rows.append(f"{lab:>12s}  {L:10.3f}")
        lmb = self.leff_minbias_AA()
        rows.append(f"{'MinBias':>12s}  {lmb:10.3f}")
        print("\n".join(rows))
