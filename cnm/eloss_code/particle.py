...
_PP_DEFAULTS: Dict[Family, Tuple[float, float, float]] = {
    "bottomonia": (6.6, 2.8, 13.8), ## 5 TeV pp fit
    "charmonia":  (4.2, 3.5, 19.2), ## 5 TeV pp fit
}

# Arleo & Peigne RHIC 200 GeV params (from arXiv:1304.0901)
_PP_RHIC_200: Dict[Family, Tuple[float, float, float]] = {
    "bottomonia": (6.4, 2.7, 8.8),  # Reference value
    "charmonia":  (3.3, 4.3, 8.3),  # AP Table 1
}
...
class PPSpectrumParams:
    ...
    @staticmethod
    def from_family(fam: Family, roots_GeV: float = 5023.0) -> "PPSpectrumParams":
        if roots_GeV < 500.0:
            p0, m, n = _PP_RHIC_200[fam]
        else:
            p0, m, n = _PP_DEFAULTS[fam]
        return PPSpectrumParams(p0, m, n)
...
class Particle:
    ...
    def pp(self) -> PPSpectrumParams:
        # For backwards compatibility with code that doesn't pass roots_GeV
        return self.pp_params if self.pp_params is not None else PPSpectrumParams.from_family(self.family, 5023.0)

    def get_pp(self, roots_GeV: float) -> PPSpectrumParams:
        if self.pp_params is not None: return self.pp_params
        return PPSpectrumParams.from_family(self.family, roots_GeV=roots_GeV)
    ...
    def _F1(self, pT_GeV, roots_GeV):
        pp = self.get_pp(roots_GeV)
        p0, m = pp.p0, pp.m
        pT = np.asarray(pT_GeV, float)
    ...
    def d2sigma_pp(self, y, pT_GeV, roots_GeV):
        ...
        val = self._F1(pT_GeV, roots_GeV) * self._F2(y, pT_GeV, roots_GeV)
