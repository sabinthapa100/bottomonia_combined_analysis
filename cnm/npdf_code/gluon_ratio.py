...
class WoodsSaxon:
    ...
    def thickness(self, b):
        ...
        return float(np.trapz(self.rho(np.hypot(b, z)), z))
    ...
    def inel_pdf(self, sigmaNN_mb=70.0):
        ...
        Z = float(np.trapz(w, b))
        w = w / Z if Z > 0 else w
        return b, T, w

    def Nnorm(self):
        b, T = self.T_grid()
        I = float(np.trapz(2 * np.pi * b * (T ** 2), b))
    ...
    def alpha_bar_for_bin(self, pL, pR, sigmaNN_mb=70.0):
        ...
        m = (b >= bl) & (b < br)
        den = float(np.trapz(w[m], b[m]))
        num = float(np.trapz((T[m] / T0) * w[m], b[m]))
        return (num / den) if den > 0 else np.nan

@dataclass
class WoodsSaxonAu:
    """Woods-Saxon parameters for Gold (Au, A=197)."""
    A: int = 197
    R: float = 6.38
    a: float = 0.535
    rho0: float = 0.17
    ...
    def inel_pdf(self, sigmaNN_mb=42.0):
        b, T = self.T_grid()
        s = self.sigma_mb_to_fm2(sigmaNN_mb)
        w = 2 * np.pi * b * (1.0 - np.exp(-s * T))
        Z = float(np.trapz(w, b))
    ...
    def alpha_bar_for_bin(self, pL, pR, sigmaNN_mb=42.0):
        b, T, w = self.inel_pdf(sigmaNN_mb)
        T0 = float(T[0])
        cdf = np.r_[0.0, np.cumsum(0.5 * (w[1:] + w[:-1]) * np.diff(b))]
