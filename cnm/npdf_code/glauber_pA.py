...
from npdf_code.glauber import _interp1
...
class HulthenProfile:
    """Orientation-averaged thickness function for the deuteron using Hulthen wave function."""
    alpha: float = 0.228  # fm^-1
    beta: float = 1.18    # fm^-1
    A: int = 2
    zmax_fm: float = 50.0
    nz: int = 400
    ...
    def rho(self, r: np.ndarray) -> np.ndarray:
        # Normalized to 1
        N2 = (self.alpha * self.beta * (self.alpha + self.beta)) / (2.0 * np.pi * (self.beta - self.alpha)**2)
        # Avoid division by zero at r=0
        r_safe = np.where(r < 1e-12, 1e-12, r)
        return N2 * (np.exp(-self.alpha * r_safe) - np.exp(-self.beta * r_safe))**2 / (r_safe**2)

    def T_d(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        """
        Thickness function T_d(b) = integral dz rho_d(sqrt(b^2+z^2))
        where rho_d(x) = 16 |psi(2x)|^2 is the nucleon density centered at CM.
        Equivalently: T_d(b) = 8 * T_{|psi|^2}(2b).
        """
        b = np.sqrt(x * x + y * y)
        b_scaled = 2.0 * b.flatten()
        ...
        T_vals = []
        for bi_s in b_scaled:
            r = np.sqrt(bi_s * bi_s + z * z)
            dens = self.rho(r)  # this is |psi(r)|^2 normalized to 1
            # T_{|psi|^2}(2b)
            T_vals.append(self.zmax_fm * np.sum(weights * dens))
            
        # T_d(b) = 8 * T_{|psi|^2}(2b)
        return 8.0 * np.array(T_vals).reshape(x.shape)
...
class OpticalGlauber:
    ...
    def __init__(self,
        ...
        if self.verbose:
            integ = np.trapz(np.trapz(self.T_xy, self.y_grid, axis=1),
                                 self.x_grid, axis=0)
            print(f"[Glauber] ∫T_A d^2x ≈ {float(integ):.3f} (target A={self.spec.A})")

        # 3) projectile
        self.proton = ProtonProfile()
        self.hulthen = HulthenProfile()

        # ------------------------------------------------------------
        # Deuteron thickness LUT: Td_r(r) needed for nuclear absorption
        # (does NOT affect existing pA behavior unless explicitly used)
        # ------------------------------------------------------------
        self.td_rmax_fm = float(max(self.bmax_fm, 20.0))
        self.td_dr_fm = 0.05  # coarse is fine; absorption uses smooth Td
        self.rTd_grid = np.arange(0.0, self.td_rmax_fm + 1e-12, self.td_dr_fm)

        # Tabulate Td(r) = ∫ dz ρ_cm(sqrt(r^2+z^2)) using the same Hulthén setup.
        # Use x=r, y=0 to evaluate the orientation-averaged thickness.
        x = self.rTd_grid
        y = np.zeros_like(x)
        self.Td_r_lut = self.deuteron.T_d(x, y).astype(float)

        if self.verbose:
            # ∫ d²s Td(s) should be ~2 for a properly normalized deuteron thickness
            integ_td = 2.0 * math.pi * np.trapz(self.rTd_grid * self.Td_r_lut, self.rTd_grid)
        ...
        if self.verbose:
            print("[Glauber] Tabulating T_AA(b), T_pA(b), T_dA(b)…")
        self.TAA_b = np.array([self._TAA_of_b(b) for b in self.b_grid])
        self.TpA_b = np.array([self._TpA_conv_of_b(b, proj="p") for b in self.b_grid])
        self.TdA_b = np.array([self._TpA_conv_of_b(b, proj="d") for b in self.b_grid])

        # Cross sections & CDFs
        self.sigma_AA_tot_mb = self._sigma_tot_mb(kind="AA")
        self.sigma_pA_tot_mb = self._sigma_tot_mb(kind="pA")
        self.sigma_dA_tot_mb = self._sigma_tot_mb(kind="dA")
        self.cum_AA = self._cdf(kind="AA")
        self.cum_pA = self._cdf(kind="pA")
        self.cum_dA = self._cdf(kind="dA")
        if self.verbose:
            print(f"[Glauber] σ_tot: AA≈{self.sigma_AA_tot_mb:.1f}, "
                  f"pA≈{self.sigma_pA_tot_mb:.1f}, dA≈{self.sigma_dA_tot_mb:.1f} mb")

    def get_T_grid(self, kind: Literal["AA", "pA", "dA"]) -> Tuple[np.ndarray, np.ndarray]:
        """Return (b, T) grid for the specified collision system."""
        if kind == "AA": return self.b_grid, self.TAA_b
        if kind == "pA": return self.b_grid, self.TpA_b
        return self.b_grid, self.TdA_b
    ...
    def Td_r(self, r: np.ndarray | float) -> np.ndarray | float:
        """
        Deuteron thickness profile T_d(r) [fm^-2], orientation-averaged Hulthén,
        normalized such that ∫ d²s T_d(s) ≈ 2.

        Required for NuclearAbsorption(mode='dA_avg_TA').
        """
        r = np.asarray(r, float)
        out = np.zeros_like(r)
        m = r <= self.rTd_grid[-1]
        out[m] = _interp1(self.rTd_grid, self.Td_r_lut, r[m])
    ...
    def _TpA_conv_of_b(self, b_fm: float, proj: Literal["p", "d"] = "p") -> float:
        bx = b_fm / 2.0
        # For deuteron, we might need a slightly wider box if T_d is broad,
        # but for consistency with pA logic, we keep it similar unless it's a bottleneck.
        ...
        def f(X: np.ndarray, Y: np.ndarray) -> np.ndarray:
            ...
            if proj == "p":
                ...
                Tp = self.hulthen.T_d(X - bx, Y)
            return Ta * Tp
        return _gl_integrate_2d(f, xa, xb, ya, yb, nx=self.nx_pa, ny=self.ny_pa)

    # --- σ_tot and CDF ---

    def _sigma_tot_mb(self, kind: Literal["AA", "pA", "dA"]) -> float:
        sigma_fm2 = self.sigma_nn_mb * MB_TO_FM2
        T = self.TAA_b if kind == "AA" else (self.TpA_b if kind == "pA" else self.TdA_b)
        integrand = self.b_grid * (1.0 - np.exp(-sigma_fm2 * T))
        val_fm2 = 2.0 * math.pi * np.trapz(integrand, self.b_grid)
        return float(val_fm2 * FM2_TO_MB)

    def _cdf(self, kind: Literal["AA", "pA", "dA"]) -> np.ndarray:
        sigma_fm2 = self.sigma_nn_mb * MB_TO_FM2
        if kind == "AA": T = self.TAA_b
        elif kind == "pA": T = self.TpA_b
        else: T = self.TdA_b
        
        integrand = self.b_grid * (1.0 - np.exp(-sigma_fm2 * T))
        cum = 2.0 * math.pi * np.cumsum(integrand) * self.db_fm
        sig_tot = self.sigma_AA_tot_mb if kind == "AA" else \
                  self.sigma_pA_tot_mb if kind == "pA" else self.sigma_dA_tot_mb
        return (cum * FM2_TO_MB) / max(sig_tot, 1e-30)

    def b_from_percentile(self, c: float, kind: Literal["AA","pA", "dA"]) -> float:
        c = float(np.clip(c, 0.0, 1.0))
        if kind == "AA": cum = self.cum_AA
        elif kind == "pA": cum = self.cum_pA
        else: cum = self.cum_dA
