...
New (minimal-change) functionality
----------------------------------
- If the passed gluon provider implements `SA_ypt_set(y, pt, set_id)`, we use set_id=1
  automatically in central-flow methods; this plugs in your `gluon_ratio.GluonEPPSProvider`.
- Matching flow for “use all gluon ratios”:
  * CentralityModel.rpa_vs_centrality_integrated_matched(...)  [existing here]
  * CentralityModel.rpa_vs_b_matched(...)                      [new]
  * CentralityModel.K_factor_ypt_b(...), K_factors_ypt_b_all_sets(...) [new]

This lets you compute RpA centrality either with the central S_A only (old behavior)
or with all EPPS sets matched one-to-one to your TopDrawer pA/pp ratio members—correct
Hessian-style bands, minimal code churn.
"""
...
def load_top_file(path: str | Path, kick: str = "pp", drop_last_pairs: int = 2) -> TopData:
    ...
    chosen = [p[sel] for p in pairs if len(p) == 2]
...
class RpAAnalysis:
    ...
    def compute_rpa_members(self, df_pp, df_pa, df_errors, join="intersect"):
        ...
        return base, r0, M  # base[['y','pt']], central ratio, member ratios
...
class CentralityModel:
    """
    RpA^npdf(y,pT;b) = [1 + N (S_A-1) T(b)/T(0)] / S_A × sigma_pA(y,pT)/sigma_pp.
    """
    gluon: object
    geom: WoodsSaxonPb
    T_grid: pd.DataFrame
    ...
    def attach_SA_to_grid(self, rgrid: pd.DataFrame, verbose: bool = False) -> pd.DataFrame:
        """
        Attach S_A(y,pt) to rgrid.
        Minimal-change enhancement:
        - If `gluon` has SA_ypt_set(y, pt, set_id), we use set_id=1 automatically.
        - Else if it has SA_ypt(y, pt), use that.
        - Else if it has make_SA_on_xy(), use the grid pa/pp fallback.
        """
        rg = rgrid.copy()

        if hasattr(self.gluon, "SA_ypt_set"):
            # EPPS provider path (central set)
            YP = rg[["y","pt"]].to_numpy(dtype=float)
            SA = np.array(self.gluon.SA_ypt_set(YP[:,0], YP[:,1], set_id=1), float)
            rg["SA"] = np.where(np.isfinite(SA) & (SA > 0), SA, 1.0)
            return rg

        if hasattr(self.gluon, "SA_ypt"):
            YP = rg[["y","pt"]].to_numpy(dtype=float)
            SA = np.array([float(self.gluon.SA_ypt(y, p)) for y, p in YP])
            if hasattr(self.gluon, "_x_grid") and getattr(self.gluon, "_x_grid", None) is not None:
        ...
        raise AttributeError("gluon provider must implement SA_ypt_set, SA_ypt, or make_SA_on_xy")
    ...
    def SAWS_ypt_b(self, y: float, pT: float, b_val: float) -> float:
        # (kept for compatibility; used by some notebooks)
        SA = float(getattr(self.gluon, "SA_ypt")(y, pT)) if hasattr(self.gluon, "SA_ypt") else \
             float(self.gluon.SA_ypt_set(np.array([y]), np.array([pT]), set_id=1).squeeze())
        alpha = self.alpha_of_b(b_val)
        return 1.0 + self.Nnorm * (SA - 1.0) * alpha

    # ---------- R_npdf factor K(b,y,pt) from EPPS (central/all sets) ----------
    def K_factor_ypt_b(self, y_arr, pt_arr, b_val, epps_provider, set_id: int = 1) -> np.ndarray:
        """
        K(b,y,pt) = [1 + N (S_A-1) α(b)] / S_A for a given set_id (default central).
        epps_provider must implement SA_ypt_set(y, pt, set_id).
        """
        y = np.asarray(y_arr, float); pt = np.asarray(pt_arr, float)
        SA = np.asarray(epps_provider.SA_ypt_set(y, pt, set_id=int(set_id)), float)
        SA = np.clip(SA, 1e-12, None)
        alpha = self.alpha_of_b(float(b_val))
        return (1.0 + self.Nnorm * (SA - 1.0) * alpha) / SA

    def K_factors_ypt_b_all_sets(self, y_arr, pt_arr, b_val, epps_provider, set_ids=None) -> Tuple[np.ndarray, np.ndarray]:
        """
        Returns (K_central, K_members) where:
          K_central: shape (Npts,)
          K_members: shape (Kmem, Npts) for set_ids (defaults to 2..107).
        """
        y = np.asarray(y_arr, float); pt = np.asarray(pt_arr, float)
        Kc = self.K_factor_ypt_b(y, pt, b_val, epps_provider, set_id=1)
        if set_ids is None:
            set_ids = list(range(2, int(getattr(epps_provider, "epps", getattr(epps_provider, "_NSET", 107))._NSET)+1))
        Kmem = []
        for sid in set_ids:
            Kmem.append(self.K_factor_ypt_b(y, pt, b_val, epps_provider, set_id=int(sid)))
        return Kc, np.asarray(Kmem, float)

    # ---------- scale a whole rgrid by α (centrality bins) ----------
    ...
    # ---------- R_pA vs impact parameter b (central-only flow preserved) ----------
    ...
    # ---------- NEW: R_pA(b) with EPPS sets matched to TopDrawer member ratios ----------
    def rpa_vs_b_matched(
        self, base_xy, r0, members, df_pa,
        b_values: Sequence[float], epps_provider, set_ids=None,
        y_min=-5, y_max=5, pt_min=0.0, pt_max=20.0,
        sigmaNN_mb=71.0, verbose=False
    ):
        """
        Matched version of rpa_vs_b:
        - base_xy: DataFrame with ['y','pt'] (from RpAAnalysis.compute_rpa_members)
        - r0: central sigma_pA/sigma_pp on base points
        - members: array (Kmem, Npoints) of member ratios (same order as TopDrawer files)
        - epps_provider: object with SA_ypt_set(y, pt, set_id) (e.g., GluonEPPSProvider)
        - set_ids: list of EPPS set IDs matching 'members' rows (defaults to 2..Kmem+1)
        """
        base = base_xy.copy()
        msel = ((base["y"]>=y_min)&(base["y"]<=y_max)&
                (base["pt"]>=pt_min)&(base["pt"]<=pt_max)).to_numpy()
        if not np.any(msel):
            return pd.DataFrame(columns=["b","r_central","r_lo","r_hi"])

        y  = base.loc[msel,"y"].to_numpy(float)
        pt = base.loc[msel,"pt"].to_numpy(float)
        w = df_pa.merge(base[msel], on=["y","pt"], how="inner")[["val"]].to_numpy(float).ravel()
        ...
        for b in b_values:
            # central
            Kc = self.K_factor_ypt_b(y, pt, b, epps_provider, set_id=1)
            Rc = r0_sel * Kc
            ...
            for j, sid in enumerate(set_ids):
                Kj = self.K_factor_ypt_b(y, pt, b, epps_provider, set_id=int(sid))
                Rj = members[j, msel] * Kj
                binned.append(float(np.sum(Rj*w)/np.sum(w)) if np.sum(w)>0 else np.nan)

            mem = np.asarray(binned, float)
            diff = mem - r_c
            err_plus  = np.sqrt(np.sum(np.square(np.maximum(diff, 0.0))))
            err_minus = np.sqrt(np.sum(np.square(np.minimum(diff, 0.0))))
            rows.append({"b": float(b),
                         "r_central": r_c,
                         "r_lo": r_c - err_minus,
                         "r_hi": r_c + err_plus})
            if verbose:
                print(f"b={b:.3f}: Rc={r_c:.4f}, +{err_plus:.4f}/-{err_minus:.4f}")
        return pd.DataFrame(rows)
    ...
    def rpa_vs_centrality_integrated(self, rgrid: pd.DataFrame, df_pa: pd.DataFrame, centrality_edges_pct: Sequence[float],
                                     ...
                                     sigmaNN_mb: float = 70.0, weight: str = "inelastic", verbose: bool = True) -> pd.DataFrame:
        if verbose:
    ...
    def rpa_vs_centrality_integrated_matched(
        ...
        for label, alpha in alphas:
            ...
            binned = []
            for j, setj in enumerate(set_ids):
                SAj = epps_provider.SA_ypt_set(y, pt, set_id=int(setj))
            ...
            err_plus  = np.sqrt(np.sum(np.square(np.maximum(diff, 0.0))))
            err_minus = np.sqrt(np.sum(np.square(np.minimum(diff, 0.0))))
            rows.append({"cent_bin": label, "r_central": r_c,
                        "r_lo": r_c - err_minus, "r_hi": r_c + err_plus})
        return pd.DataFrame(rows)
