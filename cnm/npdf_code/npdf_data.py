...
def discover_by_number(folder: str | Path, prefix: str = "") -> List[Tuple[str, int]]:
    """
    Find all .top files matching pattern *e21NN*.top or *epps21NN*.top and return sorted by NN.
    ...
    folder : str or Path
        Directory to search
    prefix : str, optional
        Prefix that the file must start with
    ...
    *e21002diff.top, *e21003diff.top, ... (pA error sets)
    """
    folder = Path(folder)
    files = sorted(folder.glob(f"{prefix}*.top"))
    pat = re.compile(r".*(?:e21|epps21)(\d+)diff\.top$", re.IGNORECASE)
...
class NPDFSystem:
    ...
    @staticmethod
    def from_folder(folder: str, kick: str = "pp", name: str = "system", prefix: str = "") -> "NPDFSystem":
        """
        ...
        name : str, default "system"
            Descriptive name
        prefix: str, default ""
            Prefix that the filename must start with 
        ...
        """
        files = discover_by_number(folder, prefix=prefix)
        if len(files) < 2:
            raise ValueError(
...
class RpAAnalysis:
    ...
    def rpa_vs_y_pt_threshold(
        ...
        *,
        ...
        merged = merged[(merged["pt"] >= float(pt_min)) & (merged["pt"] <= float(pt_max))].copy()
