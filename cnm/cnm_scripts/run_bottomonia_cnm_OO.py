...
Y_LIM_RPA = (0.5, 1.2)
X_LIM_PT = (0, 20)  # Extended to 20 GeV
...
def plot_mb_summary(cnm, yc, bands_y_mb, bands_pt_mb_list, energy):
    ...
    for i, (yname, pc, bands_pt) in enumerate(bands_pt_mb_list):
        ...
        for comp in COMPONENTS_TO_PLOT:
            ...
            ax.fill_between(
                xe,
                step_from_centers(pc, bands_pt[comp][1]["MB"])[1],
                step_from_centers(pc, bands_pt[comp][2]["MB"])[1],
                step="post", color=color, alpha=0.1, lw=0
            )
...
def run_energy(energy):
    ...
    # Minimal fix: pin edges for O+O specifically to avoid nPDF artefacts
    pin_edge_bins(bands_y, yc)
    ...
    save_csv_y(OUTDIR_MB, yc, bands_y, ["MB"], energy)
    ...
    for comp in COMPONENTS_TO_PLOT:
        ...
        ax_mb_y.fill_between(
            xe,
            step_from_centers(yc, bands_y[comp][1]["MB"])[1],
            step_from_centers(yc, bands_y[comp][2]["MB"])[1],
            step="post", color=COLORS[comp], alpha=0.1, lw=0
        )
    ...
    ax_mb_y.set_xlabel(r"$y$", fontsize=16)
    ax_mb_y.set_ylabel(r"$R_{AA}$", fontsize=16)
    ax_mb_y.minorticks_on()
    ax_mb_y.tick_params(which="both", direction="in", top=True, right=True, labelsize=12)
    ax_mb_y.legend(loc="lower left", frameon=False, fontsize=12)
    ax_mb_y.text(0.05, 0.95, rf"$\mathbf{{O+O, \sqrt{{s_{{\rm NN}}}} = {energy} \, TeV}}$ (MB)",
                 transform=ax_mb_y.transAxes, ha="left", va="top", fontsize=14)
    ...
    for y0, y1, yname in Y_WINDOWS:
        ...
        save_csv_pT(OUTDIR_MB, pc, bands_pT, ["MB"], energy, yname)
        bands_pt_mb_list.append((yname, pc, bands_pT))

    # MB Consolidated Summary
    print(f"  [PLOT] MB Consolidated Summary ...", flush=True)
    fig_mb_sum = plot_mb_summary(cnm, yc, bands_y, bands_pt_mb_list, energy)
    fig_mb_sum.savefig(OUTDIR_MB / f"Upsilon_RAA_CNM_MB_Summary_OO_{etag}TeV.pdf", bbox_inches="tight")
    fig_mb_sum.savefig(OUTDIR_MB / f"Upsilon_RAA_CNM_MB_Summary_OO_{etag}TeV.png", dpi=DPI, bbox_inches="tight")
    plt.close(fig_mb_sum)

    # MB-only pT (for multiple windows, individual)
    ...
    for i, (y0, y1, yname) in enumerate(Y_WINDOWS):
        ...
        for comp in COMPONENTS_TO_PLOT:
            ...
            lw = 2.4 if comp == "cnm" else 1.8
            ax.step(
                xe, yc_s, where="post", lw=lw, ls=ls,
                color=COLORS.get(comp, "black"), label=LABELS.get(comp, comp)
            )
            ax.fill_between(
                xe,
                step_from_centers(pc_mb, bands_pt_mb[comp][1]["MB"])[1],
                step_from_centers(pc_mb, bands_pt_mb[comp][2]["MB"])[1],
                step="post", color=COLORS.get(comp, "black"), alpha=0.1, lw=0
            )
        ...
        ax.text(
            ...
            transform=ax.transAxes,
            ha="left", va="top",
            fontsize=13, color="#202020", fontweight="bold"
        )
        ...
        if i == 0:
            ...
            ax.text(
                0.95, 0.05,
                rf"$\mathbf{{O+O, \sqrt{{s_{{\rm NN}}}} = {energy} \, TeV}}$" + "\nMB",
                transform=ax.transAxes,
