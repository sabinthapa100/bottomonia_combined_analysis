...
print("=== Step-by-Step LHC 5.02 TeV Production Script ===")
print(f"Root dir: {ROOT}")

# ------------------------------------------------------------------
# 2. Configuration (LHC p+Pb 5.02 TeV)
# ------------------------------------------------------------------
ENERGY = "5.02"
SQRTS = 5023.0
SIGMA_NN = 67.6  # mb
...
P_EDGES = np.linspace(0.0, 15.0, 31)
Y_WINDOWS = [
    (-4.46, -2.96, "Backward"),
    (-1.37, 0.43, "Mid-Rapidity"),
    (2.03, 3.53, "Forward")
]
...
OUTPUT_DIR = ROOT / "outputs" / "step_by_step_lhc"
...
spec = SystemSpec(SYSTEM, SQRTS, A=208, sigma_nn_mb=SIGMA_NN) 
...
npdf_dir = ROOT / "input" / "npdf" / "pPb5TeV"
sys_npdf = NPDFSystem.from_folder(str(npdf_dir), kick="pp", name="pPb 5.02 TeV")
ana = RpAAnalysis()
...
epps_ratio = EPPS21Ratio(A=208, path=str(epps_path))
...
# 3. Save
save_band_csv(y_cent, eloss_bands_y["loss"], labels_y, OUTPUT_DIR / "data_lhc_y_eloss.csv", "y")
save_band_csv(y_cent, eloss_bands_y["total"], labels_y, OUTPUT_DIR / "data_lhc_y_total.csv", "y")
...
fig_y, _ = plot_RpA_vs_y_components_per_centrality(
    ...
    suptitle=f"$J/\\psi$ p+Pb {ENERGY} TeV"
)
fig_y.savefig(OUTPUT_DIR / "plot_lhc_rpa_vs_y.png", dpi=150)
plt.close(fig_y)
...
for y0, y1, name in Y_WINDOWS:
    ...
    # 3. Save
    save_band_csv(pt_cent, eloss_bands_pt["loss"], labels_pt, OUTPUT_DIR / f"data_lhc_pT_{sname}_eloss.csv", "pT")
    save_band_csv(pt_cent, eloss_bands_pt["total"], labels_pt, OUTPUT_DIR / f"data_lhc_pT_{sname}_total.csv", "pT")
    ...
    fig_pt, _ = plot_RpA_vs_pT_components_per_centrality(
        ...
        suptitle=f"$J/\\psi$ p+Pb {ENERGY} TeV ( {name} )"
    )
    fig_pt.savefig(OUTPUT_DIR / f"plot_lhc_rpa_vs_pT_{sname}.png", dpi=150)
    plt.close(fig_pt)
...
for y0, y1, name in Y_WINDOWS:
    ...
    fig_cent, _ = plot_RpA_vs_centrality_components_band(
        ...
        system_label="p+Pb 5.02 TeV",
        note=f"{name}"
    )
    fig_cent.savefig(OUTPUT_DIR / f"plot_lhc_rpa_vs_cent_{sname}.png", dpi=150)
    plt.close(fig_cent)
