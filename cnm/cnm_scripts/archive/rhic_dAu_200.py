...
def run_rhic_production():
    start_time = time.time()
    energy = "200"
    outdir = Path(f"../outputs/final/RHIC_{energy}")
    outdir.mkdir(parents=True, exist_ok=True)

    print(f"=== Starting RHIC d+Au {energy} GeV Production ===")
    print(f"Output directory: {outdir}")

    # 1. Initialize
    # nb_bsamples=20 or 50 is good for testing/production speed. 
    # Use 50 for smoother bands if time permits, but 10 is fast.
    ...
    # 2. RpA vs y
    # Range -2.5 to 2.5 is typical for PHENIX/STAR coverage
    y_edges = np.linspace(-3.0, 3.0, 31) 
    pt_range = (0.0, 5.0) # pT integration range for y-dependence
    ...
    fig, _ = plot_RpA_vs_y_components_per_centrality(
        ...
        suptitle=f"$J/\\psi$ d+Au {energy} GeV: $R_{{dAu}}(y)$",
        ncols=2
    )
    ...
    # 3. RpA vs pT
    # We loop over backward, mid, forward
    # RHIC windows often: Backward (-2.2, -1.2), Mid (-0.35, 0.35), Forward (1.2, 2.2)
    p_edges = np.linspace(0.0, 10.0, 21) # 0 to 10 GeV
    windows = [(-2.2, -1.2, "Backward"), (-0.35, 0.35, "Mid-Rapidity"), (1.2, 2.2, "Forward")]
    ...
    for y0, y1, name in windows:
        ...
        fig, _ = plot_RpA_vs_pT_components_per_centrality(
            ...
            suptitle=f"$J/\\psi$ d+Au {energy} GeV: $R_{{dAu}}(p_T)$ - {name}",
            ncols=3 
        )
    ...
    # 4. RpA vs Centrality
    # Integrate over y-window and pT range (0-5 GeV for RHIC)
    print(f"\n[3/3] Computing RpA vs Centrality (pT: 0-5 GeV)...")
    pt_range_cent = (0.0, 5.0)
    
    for y0, y1, name in windows:
        print(f"  > Window: {name} ({y0} < y < {y1})")
        
        # Note: cnm_vs_centrality uses self.pt_range_avg by default if not passed?
        # Let's verify or update cnm logic. Actually cnm_vs_centrality takes y_window argument.
        # But where does it get pT range? It gets it from self.
        # We can temporarily update cnm.pt_range_avg or check if we can pass it.
        # Looking at code: cnm_vs_centrality(self, y_window) -> uses self.pt_range_avg.
        # So we should set it.
        ...
        # Plot
        # We need to unpack for the plotter
        # The plotter expects dictionaries for c/lo/hi and tuples for MB
        def _unpack(comp_key):
            # data_cent[comp] returns (Rc, Rlo, Rhi, Rc_MB, Rlo_MB, Rhi_MB) (arrays/floats)
            rc, rlo, rhi, mb_c, mb_lo, mb_hi = data_cent[comp_key]
            # Convert arrays to dict keyed by label
