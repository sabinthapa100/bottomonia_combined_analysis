...
# Colors (consistent with CNM script)
COLORS = {
    'cnm':          '#7B2D8B',       # purple
    'prim_NPWLC':   '#FF8C00',       # orange
    'prim_Pert':    '#00BFFF',       # deep sky blue
    'total_NPWLC':  '#DC143C',       # crimson
...
def combine_and_export_vs_y(cnm, band_npwlc, band_pert):
    ...
    # Get CNM values (same for all states)
...
def combine_and_export_vs_pt(cnm, band_npwlc, band_pert):
    ...
    for y0, y1, y_label in Y_WINDOWS:
        # Get CNM values for this y window (same for all states)
...
def plot_raa_vs_y(y_centers, combined_data):
    ...
    for col, state in enumerate(STATES):
        ...
        # Plot CNM
        cnm_c, cnm_lo, cnm_hi = data["cnm"]
        xe, yc_s = step_from_centers(y_centers, cnm_c)
        ax.step(xe, yc_s, where="post", lw=2.2, color=COLORS["cnm"], label=LABELS["cnm"])
        ax.fill_between(xe, *step_from_centers(y_centers, cnm_lo)[1:], step="post", 
                         color=COLORS["cnm"], alpha=ALPHA_BAND, lw=0)
...
def plot_raa_vs_pt_grid(pt_centers, combined_data):
    ...
    for row, (y0, y1, y_label) in enumerate(Y_WINDOWS):
        ...
        for col, state in enumerate(STATES):
            ...
            # Plot CNM (same for all states in this window)
            if col == 0:
                cnm_c, cnm_lo, cnm_hi = data["cnm"]
                xe, yc_s = step_from_centers(pt_centers, cnm_c)
                ax.step(xe, yc_s, where="post", lw=2.0, color=COLORS["cnm"], label=LABELS["cnm"])
                ax.fill_between(xe, *step_from_centers(pt_centers, cnm_lo)[1:], step="post",
                               color=COLORS["cnm"], alpha=ALPHA_BAND, lw=0)
